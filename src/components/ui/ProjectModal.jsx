        ...
        <>
          ...
          >
            <motion.div
              ...
              className="ui-surface rounded-xl overflow-hidden"
            ...
            >
              <div className="sticky top-0 z-10 border-b border-white/[0.06] bg-[#111714] px-5 py-4 sm:px-6">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-mono text-[10px] uppercase tracking-wider text-muted">Case study</p>
                    <h2 id="project-modal-title" className="mt-1 text-xl font-semibold text-foreground sm:text-2xl">
                      {title}
                    </h2>
              ...
              <div className="px-5 py-5 sm:px-6 sm:py-6">
                <div className="mb-6 flex flex-wrap gap-x-4 gap-y-1">
                  {project.stack.map((tech) => (
                    <span key={tech} className="font-mono text-[11px] text-muted">{tech}</span>
                  ))}
                </div>

                <dl className="space-y-0">
                  {sections.map(({ key, label, gold }) => (
                    <div key={key} className="grid gap-2 border-t border-white/[0.06] py-4 first:border-t-0 first:pt-0 sm:grid-cols-[100px_1fr]">
                      <dt className={`font-mono text-[10px] uppercase tracking-wider ${gold ? 'text-gold' : 'text-muted'}`}>{label}</dt>
                ...
                {project.id === 'hire-os' && <div className="mt-8"><HireOsDemo embedded /></div>}
                {project.id === 'argus' && <div className="mt-8"><ArgusDemo embedded /></div>}
                ...
                <div className="mt-8 flex flex-wrap gap-3 border-t border-white/[0.06] pt-6">
                  ...
                  {project.id === 'argus' && project.links?.marketplace && (
                    <Button variant="secondary" href={project.links.marketplace}>Marketplace ↗</Button>
                  )}
